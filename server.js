...
//   });
  ...
  app.post('/poster', (req, res) => {
    ...
    app.post('/carType',(req,res)=>{
        ...
        con.query(`select driver_ID,driver_key,latitude,longitude from u384203065_cabserve.driver where driver_status='online' and medium=${medium} and latitude<=${radiusLatPlus} and latitude>=${radiusLatMinus}  order by day_trip asc`,(err,result)=>{
          if(err){
            console.log(err)
          }else{  
            if(result[0]==null){
              ...
              console.log(driver_ID + ' is booked for a ride')
                          con.query(`SELECT COUNT(trip_ID) AS cot FROM u384203065_cabserve.trips`,(err,resul)=>{
                            if(err){
                                console.log(err)
                            }else{
                                ...
                                con.query(`insert into u384203065_cabserve.trips values(1,'${tript_idNext}','${pickaddress}','${dropaddress}',${driver_ID},'${todayDate}','started',${pinLatitude},${pinlongitude},${dropLatitude},${dropLongitude},0)`,(err,resu)=>{
                                  if(err){
                                     console.log(err) 
                                  }else{
    ...
    app.post('/driver',(req,res)=>{
      ...
      con.query(`select driver_status from u384203065_cabserve.driver where driver_ID=${driver_ID}`,(err,result)=>{
        if(err){
          console.log(err)
        }else{
          const driverSta=result[0].driver_status
                                  ...
                                  //go offline
                                    if(driverSta=='online'){
                                      con.query(`update u384203065_cabserve.driver set driver_status='offline', driver_key='${driver_key}' where driver_ID=${driver_ID}`,(err,result)=>{
                                        if(err){
                                          console.log(err)
                                        } else{
                                          res.send({driver_key,driver_status:'offline'});
                                          // console.log({driver_key,driver_status:'online'})
                                          console.log(`data updated u384203065_cabserve.driver with ID= ${driver_ID} offline`)
                                        }
                                      })
    
                                      //go onnline
                                    }else if(driverSta=='offline'){
                                      con.query(`update u384203065_cabserve.driver set driver_status='online', driver_key='${driver_key}' where driver_ID=${driver_ID}`,(err,result)=>{
                                        if(err){
                                          console.log(err)
                                        } else{
    ...
    app.post('/driverStatus',(req,res)=>{
      ...
      console.log(req.body)
      con.query(`select driver_status,driver_key from u384203065_cabserve.driver where driver_ID=${driver_ID} `,(err,result)=>{
        if(err){
          console.log(err)
        }else{  
          console.log(result[0].driver_status)
                        if(result[0].driver_status=='online'){
                          con.query(`update u384203065_cabserve.driver set driver_key='${driver_key}' where driver_ID=${driver_ID} `,(err,result)=>{
                            if(err){
                              console.log(err)
                            }else{
    ...
    app.post('/driverDetail',(req,res)=>{
      ...
      con.query(`select * from u384203065_cabserve.driver where driver_ID=${driver_ID}`,(err,result)=>{
        if(err){
          console.log(err)
        } else{
          res.send(result)
    ...
    const driver_ID=req.body.d_ID
      con.query(`update u384203065_cabserve.driver set driver_status='${rideStatus}' where driver_ID=${driver_ID}`,(err,result)=>{
        if(err){
          console.log(err)
        } else{
          res.send(result)
    ...
    app.post('/location',(req,res)=>{
      ...
      const driver_ID = req.body.d_ID
      con.query(`update u384203065_cabserve.driver set latitude=${latitude}, longitude=${longitude} where driver_ID=${driver_ID}`,(err,result)=>{
        if(err){
          console.log(err)
        } else{
    ...
    app.post('/total',(req,res)=>{
      ...
      // if(distance<)
      console.log(calDis)
      con.query(`select trip_ID,picklatitude,picklongitude,droplatitude,droplongitude from u384203065_cabserve.trips where trip_ID='${trip_ID}'`,(err,result)=>{
        if(err) throw err
        console.log(result)
    ...
    console.log(req.body)
      if('post'==req.body.post){
        con.query(`update u384203065_cabserve.driver set day_trip=0 where driver_ID>0;`,(err,result)=>{
          if(err){
            console.log(err)
          } else{
            res.send(result)
